...
func (s *Reconciler) Create(ctx context.Context) error {
	...
	if s.scope.MachineConfig.UserDataSecret == nil {
		...
		vmExtSpec := &virtualmachineextensions.Spec{Name: "startupScript", VMName: s.scope.Machine.Name, ScriptData: base64.StdEncoding.EncodeToString([]byte(scriptData))}
...
func (s *Reconciler) Delete(ctx context.Context) error {
	...
	networkInterfaceSpec := &networkinterfaces.Spec{Name: fmt.Sprintf("%s-nic", s.scope.Machine.Name), VnetName: azure.GenerateVnetName(s.scope.Cluster.Name)}
...
func (s *Reconciler) isNodeJoin() (bool, error) {
	...
	case v1alpha1.ControlPlane:
		for _, cm := range clusterMachines.Items {
			...
			vmExtSpec := &virtualmachineextensions.Spec{Name: "startupScript", VMName: cm.Name}
...
func (s *Reconciler) isVMExists(ctx context.Context) (bool, error) {
	...
	if s.scope.MachineConfig.UserDataSecret == nil {
		vmExtSpec := &virtualmachineextensions.Spec{Name: "startupScript", VMName: s.scope.Name()}
	...
	for {
		...
		for _, node := range nodeList.Items {
			if strings.Contains(node.Spec.ProviderID, instanceID) {
				return &apicorev1.ObjectReference{Kind: node.Kind, APIVersion: node.APIVersion, Name: node.Name}, nil
			}
		}
...
func (s *Reconciler) createVirtualMachine(ctx context.Context, nicName string) error {
	...
	if err != nil && vmInterface == nil {
		...
		vmSpec = &virtualmachines.Spec{Name: s.scope.Machine.Name, NICName: nicName, SSHKeyData: string(decoded), Size: s.scope.MachineConfig.VMSize, OSDisk: s.scope.MachineConfig.OSDisk, Image: s.scope.MachineConfig.Image, Zone: vmZone}
